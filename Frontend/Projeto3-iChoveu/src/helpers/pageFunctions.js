...
export function createCityElement(cityInfo) {
  ...
  const botao = createElement('button', 'city-forecast-button');
  botao.innerHTML = 'Ver previsão';
...
const diasForecast = (url) => {
  const TOKEN = import.meta.env.VITE_TOKEN;
  const URL = `http://api.weatherapi.com/v1/forecast.json?lang=pt&key=${TOKEN}&q=${url}&days=7`;
  fetch(URL)
    .then((response) => response.json())
    .then((data) => formatarDias(data.forecast.forecastday))
    .then((dias) => showForecast(dias));
};
...
const criarSemanaDias = () => {
  const botaoPrevisao = document.querySelectorAll('.city-forecast-button');

  botaoPrevisao.forEach((botao) => {
    botao.addEventListener('click', (event) => {
      const url = event.target.attributes['data-url'].nodeValue;
      diasForecast(url);
    });
  });
};
...
export async function handleSearch(event) {
  ...
  const resultado = await searchCities(searchValue);

  resultado.forEach((data) => {
    const weather = getWeatherByCity(data.url);
    array.push(weather);
  });

  const weathers = await Promise.all(array);
  const cidades = document.querySelector('#cities');
  ...
  resultado.forEach((data, index) => {
    ...
    cidades.appendChild(createCityElement(cityElement));
  });
